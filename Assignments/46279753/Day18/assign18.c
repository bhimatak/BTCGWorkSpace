		...
		//} 
		 struct node* copyList(struct node* start) 
		 {
			 ...
			 
			{
				...
				newNode->string=start->string; newNode->next=copyList(start->next);
			       	return newNode; 
		 ...
		 { 
			 FILE *fp; char word[20]; char w[20]; 
			 fp = fopen(argv[1], "r");   
...
fgets(w,20,stdin); search_delete(w); 
display(orig); 
