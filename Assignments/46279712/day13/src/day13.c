...
extern char* concatenate_servername(char* server_name[],int count);
...
char* concatenate_servername(char* server_name[],int count)
{
        char* result=NULL;
...
int main(int argc,char* argv[])
{
	...
	if(argc>=4)
	{
		for(i=1;i<argc;i++)
		{
		   ...
		   concatenated_servernames=concatenate_servername(server_name,count_valid_names);
