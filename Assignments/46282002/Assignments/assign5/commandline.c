...
char* concatenate_servernames(char* server_names[],int count)
{
	char* result=NULL;
	int sizeof_result=0,i;
	for(i=0;i<count;i++)
	{
		sizeof_result+=strlen(server_names[i]);
	}
	result=(char*)calloc(sizeof_result + count, sizeof(char));
	for(i=0;i<count;i++)
	{
		if(i==0)
			strcpy(result,server_names[i]);
		else
			strcat(result,server_names[i]);
		if(i!= count-1)
			strncat(result,"_",15);
		return result;
	}
	int main(int argc,char* argv[])
	{
		...
		if(argc>=4)
		{
			...
			printf("\n\n concateted server name:%s\n",concatenated_servernames);
		}
		else
		{
			printf("Enter atleast 3 arg\n");
		}
		return EXIT_SUCCESS;
	}
